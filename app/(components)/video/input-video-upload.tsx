...
import { LINKT_ABI, UPLOAD_URL, uploadToS3 } from "@/(lib)/utils";
...
async function calculateMD5(file: File): Promise<string> {
  return new Promise<string>((resolve, reject) => {
    ...
    reader.onerror = (event) => {
      reject(
        new Error(`File could not be read: ${event.target?.error?.message}`)
      );
    };
...
export const audiences: EAudience[] = [
  EAudience.ALL,
  EAudience.HOLDERS,
  EAudience.TRAIT,
  EAudience.TOKEN,
...
export function UploadVideo() {
  ...
  const { data: publicationCount } = useContractRead({
    ...
    address: process.env.FEATURE_DEPLOYED_CONTRACT_ADDRESS as `0x${string}`,
  }) as any;
  ...
  const { config } = usePrepareContractWrite({
    ...
    args: [
      +publicationCount + 1, // VideoId - always just increment what is existing
      fileHash,
      connectedContract,
      selectedAudience,
    ],
  ...
  const handleUpload = async () => {
    ...
    if (placeholderData.file) {
      ...
      if (isSuccess) {
        ...
        const placeholderPresignUrlData = JSON.parse(
          await placeholderPresignUrl.json()
        );
        await uploadToS3(
          placeholderPresignUrlData.uploadUrl,
          placeholderData.file,
          placeholderData.file.type,
          setProgress
        );
      }
    }
    try {
      ...
      if (response?.ok) {
        ...
        const revalidate = await fetch(
          "/dashboard/api/revalidate?tag=discovery&secret=foobar",
          {
            method: "POST",
          }
        );
  ...
  return (
    <div className="grid w-full lg:max-w-sm items-center gap-1.5 space-y-6">
      ...
      {uploadStatus ? (
        <div className="text-sm text-white">{uploadStatus}</div>
      ) : null}
    </div>
  );
}
