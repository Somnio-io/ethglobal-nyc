...
import { ObjectMetadata, User, authUserToken, createApiGatewayResponse, getItem, getPreSignedUrl, mimeTypeToExtension } from "./utils";
...
export const createUploadPresign: APIGatewayProxyHandler = async (event, context) => {
  ...
  if (!contentType) {
    return createApiGatewayResponse(400, JSON.stringify({ message: "Unknown File Type" }));
  }
  const extension = mimeTypeToExtension(contentType);

  const user = await getItem<User>(USERS_TABLE_NAME, { address: lowerCaseAddress }, docClient);
  ...
  if (user) {
    ...
    uploadUrl = await getPreSignedUrl(BUCKET_NAME, objectKey, expiresIn, s3, "PUT", metadata);
  }

  if (user) {
    return createApiGatewayResponse(200, JSON.stringify({ message: "OK", uploadUrl }));
  }
  return createApiGatewayResponse(404, JSON.stringify({ message: "Not Found" }));
};
